...
- **四类音源，随心切换**
  ...
  - 🌐 **在线音乐**：搜索 [Audius](https://audius.co) 免费音乐库（免 key、可完整播放），点结果即可加入曲库 / 直接播放，并**自动尝试匹配歌词**
...
| 配置 Audius API Key | 在「🌐 在线音乐」弹窗内展开「⚙ Audius API Key」填写（可选，提升速率配额）。**只填 API Key —— Bearer Token 是后端专用凭据，不要填进网页** |
...
- **Audius 凭据（重要）**：Audius 给开发者两种凭据 —— **API Key 可放前端**（官方允许，仅用于提升速率配额，可在弹窗内填写并只存在本机）；**Bearer Token 是后端专用，官方明确禁止放进浏览器 / 移动端代码**，本项目不收集、不保存它。连接个人账号需要 OAuth 2.0 PKCE 流程，纯前端版本未做该集成。
