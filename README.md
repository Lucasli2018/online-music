...
- **四类音源，随心切换**
  ...
  - 🌐 **在线音乐**：搜索 [Audius](https://audius.co) 免费音乐库（免 key、可完整播放），结果标注歌词可用性，点结果即可加入曲库 / 直接播放；填 API Key 可提升速率配额；还支持**用 Audius 账号登录**（OAuth 2.0 PKCE，只读）载入自己发布的曲目
...
│   ├── samples.js       # 内置示例曲数据
│   ├── lyrics.js        # LRC 解析/同步/编码探测/在线匹配
│   ├── online.js        # 在线音乐音源（Audius 搜索 + 流地址）
...
| 在线音乐 | 点「🌐 在线音乐」，输入关键词搜索 Audius 免费曲库；点结果行 = 加入并播放，点「＋ 加入」= 只入库；每条结果会标注 LRCLIB 是否有歌词 |
...
- **在线音乐依赖 Audius**：「🌐 在线音乐」对接 [Audius](https://audius.co) 公共 API（免 key、CORS 开放、可完整播放），需联网；其为境外服务，网络受限时可能搜索失败或加载缓慢。入库的在线曲目**只保存流地址**，不含音频文件，因此与远程歌一样受「备份不含音频」限制。
...
- **Audius 不提供歌词**：Audius 接口只返回音频与元数据（歌名 / 艺术家 / 时长 / 流派），**没有任何歌词字段**。所以歌词只能来自第三方 —— 本项目用 [LRCLIB](https://lrclib.net) 按「歌名 + 艺术家」匹配。
