...
- **四类音源，随心切换**
  ...
  - 🌐 **在线音乐（多音源）**：一处切换三个免费音源，结果标注歌词可用性，点结果即可加入曲库 / 直接播放
...
- **歌词（LRC）**：自动解析 LRC 并随播放进度滚动高亮；上传歌曲时若同时选中同名的 `.lrc` 文件会**自动关联**；点歌词面板任意一行可**跳转到对应时间**；也支持手动粘贴编辑、远程链接填 LRC 地址。解析器兼容一行多时间标签、`[offset:]` 整体时间偏移，自动忽略 `[ti:]`/`[ar:]` 等元数据标签；并支持**逐字卡拉OK**（增强格式内联字级时间标签，随唱逐字点亮）与**双语歌词**（同时间戳双行自动合并原词+翻译）。歌词面板「🔍 匹配」可**在线自动匹配**歌词（对接 LRCLIB 公共服务，需联网）
...
│   ├── samples.js       # 内置示例曲数据
│   ├── lyrics.js        # LRC 解析/同步/编码探测/在线匹配（LRCLIB + lyrics.ovh 兜底）
│   ├── online.js        # 在线音乐多音源调度（音源注册表 + 内置 Audius 源）
│   ├── sources.js       # 追加音源：Jamendo（CC 完整曲目）、iTunes（30 秒试听）
...
| 在线音乐 | 点「🌐 在线音乐」，先选音源（Audius / Jamendo / iTunes 试听），输入关键词搜索；点结果行 = 加入并播放，点「＋ 加入」= 只入库；每条结果标注 LRCLIB 是否有歌词（iTunes 标注「30秒试听」） |
| 切换音源 | 弹窗顶部音源标签点选即可，选择会记住；Audius 专属的 API Key / 账号区仅在该源下显示 |
...
| 在线匹配歌词 | 选中一首歌后，点歌词面板「🔍 匹配」，按歌名/歌手自动获取（需联网；失败可手动编辑） |
...
- **在线歌词匹配依赖第三方服务**：「🔍 匹配」功能对接 [LRCLIB](https://lrclib.net) 公共服务，需要联网；该服务为社区维护，覆盖以常见曲目为主，匹配不到时请用「✎ 编辑」手动粘贴 LRC。
